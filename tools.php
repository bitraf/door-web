...
if ($ok)
{
  @pg_query_params("INSERT INTO auth_log (host, account, realm) VALUES ($1, $2, 'tool-a001')", array($_SERVER['REMOTE_ADDR'], $account));
  @pg_query_params('INSERT INTO checkins (account) VALUES ($1)', array($account));
}
else if ($rate_limited)
{
  @pg_query_params("INSERT INTO auth_log (host, realm) VALUES ($1, 'tool-a001')", array($_SERVER['REMOTE_ADDR']));
}
?>
<!DOCTYPE HTML>
<html>
  ...
  <?php if ($ok): ?>
	...
	<div class='alert alert-success'>
		<h4>Tool lock is open!</h4>Put it back after use! 
...
if ($ok)
  system('/usr/local/bin/bitraf-toollock-open.sh '.escapeshellarg($_POST['user']));
